...
func TestIsValidLocation(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "Valid location (row 0, col 1)", location: csv.Location{RowIndex: 0, ColIndex: 1}, shouldPass: true},
		{name: "Valid location (row 1, col 0)", location: csv.Location{RowIndex: 1, ColIndex: 0}, shouldPass: true},
		{name: "Invalid row index (negative)", location: csv.Location{RowIndex: -1, ColIndex: 0}, shouldPass: false},
		{name: "Invalid column index (negative)", location: csv.Location{RowIndex: 0, ColIndex: -1}, shouldPass: false},
		{name: "Row index out of bounds", location: csv.Location{RowIndex: 2, ColIndex: 0}, shouldPass: false},
		{name: "Column index out of bounds", location: csv.Location{RowIndex: 0, ColIndex: 2}, shouldPass: false},
	}

	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			err := csv.IsValidLocation(test.location, csvData)
